...
const config = {
  ...
  sourceDir:
    process.argv[2] || path.join(__dirname, "../frontend/public/images"),
  outputDir:
    process.argv[3] ||
    path.join(__dirname, "../frontend/public/images/optimized"),
...
async function processImage(inputPath, outputDir) {
  ...
  try {
    ...
    for (const size of settings.sizes) {
      ...
      const aspectRatio =
        settings.aspectRatio.width / settings.aspectRatio.height;
      let width = size;
    ...
    if (originalFormat && config.formats.includes(originalFormat)) {
      ...
      try {
        ...
        console.error(
          `  ✗ Failed to create original ${outputFileName}:`,
          err.message
        );
      }
    }
